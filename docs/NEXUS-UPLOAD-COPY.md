...
```text
A sleek, read-only Project Zomboid interactive map with game-derived towns, streets, businesses, filters, coordinates, navigation, and custom markers.
```
...
```text
Knox Atlas 0.1.0 — Windows Installer (Recommended)
```

Suggested file description:

```text
Recommended for most players. Download and run the setup EXE, follow the Windows prompts, then launch Knox Atlas from its normal shortcut. This installs the app and adds a standard Windows uninstall entry. You do not need the portable ZIP as well.
...
```text
Knox Atlas 0.1.0 — Portable Windows ZIP
```

Suggested file description:

```text
Optional no-installer package. Download the ZIP, choose Extract All, move the extracted folder somewhere writable, and run Knox-Atlas.exe from that folder. Do not run it from inside the ZIP preview, and keep the included files together.
